...
**Function calling** provides a powerful and flexible way for LLM models to interface with your code or external services, and has two primary use cases:
...
**Model decides to call tool(s)** – model returns the **name** and **input arguments**.
...
```python
messages.append(completion.choices[0].message)  # append model's tool call message
...
```javascript
messages.push(completion.choices[0].message); // append model's tool call message
